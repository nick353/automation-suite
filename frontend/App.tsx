...
import { analyzeRequest, sendChatMessage, generateFinalWorkflow } from './services/geminiService';
...
function App() {
  ...
  const stripEmphasis = (text: string) => text.replace(/[＊*]/g, '');

  const handleAnalyze = async (request: GenerationRequest) => {
    setCurrentRequest(request);
    setStatus(AppStatus.ANALYZING);
    
    const urlArray = request.useExternalInfo 
      ? request.externalUrls.split('\n').map(u => u.trim()).filter(u => u.length > 0)
      : [];

    try {
      const initialPlanRaw = await analyzeRequest(request.prompt, urlArray, language, request.videoFile);
      const initialPlan = stripEmphasis(initialPlanRaw);
      
      setChatHistory([
        { role: 'user', content: request.prompt || "Here is the automation requirement (and video if attached)." },
        { role: 'model', content: initialPlan }
      ]);
  ...
  const handleSendMessage = async (message: string) => {
    if (status !== AppStatus.PLANNING) return;
    ...
    try {
      const replyRaw = await sendChatMessage(newHistory, message, language);
      const reply = stripEmphasis(replyRaw);
  ...
  const handleConfirmPlan = async () => {
    ...
    try {
      const data = await generateFinalWorkflow(chatHistory, currentRequest.topK, language);
  ...
  const showInput = status === AppStatus.IDLE || status === AppStatus.ERROR || (status === AppStatus.ANALYZING && chatHistory.length === 0);
  const showChat = (status === AppStatus.PLANNING || (status === AppStatus.ANALYZING && chatHistory.length > 0) || status === AppStatus.GENERATING);
  const showResult = status === AppStatus.SUCCESS;
