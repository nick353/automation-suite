...
type TranslationKey = 
  ...
  | 'result.strategy'
  | 'result.patterns'
  | 'result.workflowTree.title'
  | 'result.workflowTree.empty'
  | 'result.copy'
  | 'result.copied'
  | 'deploy.step'
  | 'deploy.title'
...
export const translations: Record<Language, Record<TranslationKey, string>> = {
  en: {
    ...
    'result.workflowTree.title': 'Workflow Steps',
    'result.workflowTree.empty': 'No workflow steps detected in the generated JSON.',
    'result.copy': 'COPY',
    ...
    'deploy.error.config': 'Missing n8n credentials. Please set VITE_N8N_API_URL and VITE_N8N_API_KEY.',
    'deploy.error.deployFailed': 'Deployment failed. Please check server logs or credentials.',
  ...
  ja: {
    ...
    'result.workflowTree.title': 'ワークフローの流れ',
    'result.workflowTree.empty': '生成されたJSON内にステップが見つかりませんでした。',
    'result.copy': 'コピー',
    'result.copied': '完了',
    'deploy.step': '3',
    'deploy.title': 'デプロイ',
    ...
    'deploy.error.config': 'n8nの接続情報が不足しています。VITE_N8N_API_URL と VITE_N8N_API_KEY を設定してください。',
    'deploy.error.deployFailed': 'デプロイに失敗しました。サーバーログまたは認証情報を確認してください。',
