...
import { deployWorkflow } from '../services/apiClient';
...
const DeployCard: React.FC<DeployCardProps> = ({ canDeploy, workflowJson, language }) => {
  ...
  const [n8nApiUrl, setN8nApiUrl] = useState<string>(import.meta.env.VITE_N8N_API_URL || '');
  const [n8nApiKey, setN8nApiKey] = useState<string>(import.meta.env.VITE_N8N_API_KEY || '');

  const handleDeploy = async () => {
    if (!workflowJson) {
      setError('Workflow JSON is missing.');
    ...
    try {
      const response = await deployWorkflow({
        ...
        n8nApiUrl: n8nApiUrl || undefined,
        n8nApiKey: n8nApiKey || undefined,
      });

      if (response.success) {
        setWorkflowId(response.n8nWorkflowId ?? workflowId ?? null);
          ...
          {status === DeployStatus.SUCCESS ? (
            ...
            >
              ...
              <div className="flex w-full items-center justify-between rounded-xl border border-green-200 bg-green-50/50 px-4 py-3">
                 ...
                 <a href="#" className="flex items-center gap-1 text-sm font-bold text-green-700 hover:underline">
             ...
             <div className="flex flex-col items-center gap-6 w-full max-w-md">
                ...
                <div className="w-full space-y-3 text-left">
                  <input
                    ...
                    placeholder="n8n API URL (optional)"
                    className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 shadow-sm focus:border-brand-400 focus:outline-none"
                  />
                  <input
                    type="password"
                    value={n8nApiKey}
                ...
                <button
                  ...
                  className={clsx(
                      ...
                      status === DeployStatus.DEPLOYING || !workflowJson
                        ? "cursor-not-allowed opacity-80"
                        : "hover:shadow-2xl hover:shadow-n8n-primary/30"
                  )}
                >
                  <div className={clsx(
                ...
                {status === DeployStatus.ERROR && (
                     <div className="flex items-center gap-2 rounded-lg bg-red-50 px-4 py-2 text-xs font-bold text-red-600">
                         ...
                         <span>{error || 'Connection refused. Please check your n8n API configuration.'}</span>
                     </div>
                )}
             </div>
          )}
